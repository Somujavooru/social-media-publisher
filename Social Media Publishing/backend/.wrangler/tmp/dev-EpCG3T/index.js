...
// .wrangler/tmp/bundle-oOgzZh/strip-cf-connecting-ip-header.js
  ...
  if (platform2 === "linkedin" || platform2 === "facebook" || platform2 === "instagram") {
    ...
    try {
      ...
      if (platform2 === "facebook" || platform2 === "instagram") {
        ...
        const tokenResponse = await fetch(`https://graph.facebook.com/v19.0/oauth/access_token?${params.toString()}`);
        const raw2 = await tokenResponse.text();
        console.log("FACEBOOK TOKEN RAW RESPONSE:", raw2);
        let data;
        try {
          data = JSON.parse(raw2);
        } catch (e) {
          console.error("Failed to parse Facebook response:", raw2);
          return new Response("Invalid JSON from Facebook", { status: 500 });
        }
        if (!data.access_token) {
          console.error("Token missing in response:", data);
          return new Response(JSON.stringify({ error: "Token missing", details: data }), {
            status: 500,
            headers: { "Content-Type": "application/json" }
          });
        }
        console.log("FACEBOOK_USER_TOKEN:", data.access_token);
        return c.json({ token: data.access_token });
        if (!tokenResponse.ok) {
          console.error(`${platform2} token error:`, data);
          return c.redirect(`${frontendUrl}/platforms?error=token_exchange_failed`);
        }
        const userId = "mock-user";
...
// .wrangler/tmp/bundle-oOgzZh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-oOgzZh/middleware-loader.entry.ts
