...
// .wrangler/tmp/bundle-Dmu7ui/checked-fetch.js
...
// .wrangler/tmp/bundle-Dmu7ui/strip-cf-connecting-ip-header.js
...
platformsRouter.post("/connect", async (c) => {
  ...
  ).bind(platformId, user.id, name, credentialsRef).run();
...
platformsRouter.delete("/:id", async (c) => {
  ...
  if (platform) {
    ...
    await c.env.DB.prepare("DELETE FROM Platforms WHERE id = ? AND user_id = ?").bind(id, user.id).run();
...
var LinkedInAdapter = class {
  async publish(params) {
    ...
    return {
      success: true,
      platform: "LinkedIn",
      externalUrn: `urn:li:ugcPost:${Date.now()}`,
      publishedAt: (/* @__PURE__ */ new Date()).toISOString()
    };
  }
};
...
var app = new Hono2();
app.use("*", cors());
app.get("/", (c) => c.json({ status: "ok", service: "Social Publishing API" }));
...
// .wrangler/tmp/bundle-Dmu7ui/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Dmu7ui/middleware-loader.entry.ts
