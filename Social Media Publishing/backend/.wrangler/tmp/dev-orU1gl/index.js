...
// .wrangler/tmp/bundle-gvX0wq/strip-cf-connecting-ip-header.js
...
var XAdapter = class {
  async authenticate(credentials) {
    return !!(credentials && credentials.apiKey && credentials.accessToken);
  }
  async publish(params) {
    ...
    if (!await this.authenticate(credentials)) {
      throw new Error(
        "Invalid X credentials. Ensure API Key and Access Token are provided."
      );
    }
    try {
      const mediaIds = [];
...
--${boundary}--`);
      ...
      if (!tweetRes.ok) {
        ...
        if (tweetRes.status === 403) {
          throw new Error(
            `X API Permission Error (403): Your app may have "Read-only" access. Go to X Developer Portal \u2192 App Settings \u2192 User authentication \u2192 change to "Read and Write". API response: ${JSON.stringify(tweetData)}`
          );
        }
        throw new Error(`X tweet failed (${tweetRes.status}): ${JSON.stringify(tweetData)}`);
      }
...
var FacebookAdapter = class {
  async authenticate(credentials) {
    return !!(credentials && credentials.accessToken && credentials.pageId);
  }
  async publish(params) {
    const { content, mediaId, credentials } = params;
    const { accessToken, pageId } = credentials;
    if (!await this.authenticate(credentials)) {
      throw new Error("Invalid Facebook credentials. Page Access Token and Page ID are required.");
    }
...
oauthRouter.get("/:platform/connect", async (c) => {
  ...
  if (platform === "facebook" || platform === "instagram") {
    const clientId = platform === "facebook" ? c.env.FACEBOOK_CLIENT_ID : c.env.INSTAGRAM_CLIENT_ID;
    const redirectUri = encodeURIComponent(`http://localhost:8787/api/oauth/${platform}/callback`);
    const scope = platform === "facebook" ? encodeURIComponent("pages_manage_posts,pages_read_engagement,public_profile") : encodeURIComponent("instagram_basic,instagram_content_publish,pages_read_engagement");
    const state = crypto.randomUUID();
...
oauthRouter.get("/:platform/callback", async (c) => {
  ...
  if (platform === "linkedin" || platform === "facebook" || platform === "instagram") {
    ...
    try {
      ...
      if (platform === "facebook" || platform === "instagram") {
        const clientId = platform === "facebook" ? c.env.FACEBOOK_CLIENT_ID : c.env.INSTAGRAM_CLIENT_ID;
        const clientSecret = platform === "facebook" ? c.env.FACEBOOK_CLIENT_SECRET : c.env.INSTAGRAM_CLIENT_SECRET;
        const redirectUri = `http://localhost:8787/api/oauth/${platform}/callback`;
        ...
        const tokenData = await tokenResponse.json();
        if (!tokenResponse.ok) {
          console.error(`${platform} token error:`, tokenData);
          return c.redirect(`${frontendUrl}/platforms?error=token_exchange_failed`);
        }
        const userId = "mock-user";
        const platformName = platform === "facebook" ? "Facebook" : "Instagram";
        let credentials = { accessToken: tokenData.access_token };
        if (platform === "facebook") {
          ...
          if (pagesData.data && pagesData.data.length > 0) {
            credentials.pageId = pagesData.data[0].id;
            credentials.accessToken = pagesData.data[0].access_token;
          }
        } else {
          const igRes = await fetch(`https://graph.facebook.com/v19.0/me/accounts?fields=instagram_business_account&access_token=${tokenData.access_token}`);
...
var AIService = class {
  ...
  async generatePlatformSpecificContent(topic, platforms, vibeOverride) {
    ...
    const baseIdentity = `You are a professional social media content generator. You must output strictly valid JSON with keys: linkedin, x, instagram, facebook, threads, whatsapp. CRITICAL RULE: The user has two distinct professional personas: (1) An Embedded Software Engineer/Student, and (2) A Decorative Lighting Business Owner. ANALYZE the topic before generating. If the topic includes tech keywords (AMD, AI, coding, software, hardware, hackathon, robotics), you MUST adopt a strict Software Engineering persona and DO NOT mention lighting, decor, or sustainability. If the topic includes business keywords (brass, shipments, showroom, interior, lights), adopt the Lighting Business persona.`;
...
publishRouter.post("/", async (c) => {
  ...
  for (const pName of platforms) {
    const postPlatformId = crypto.randomUUID();
    try {
      const dbName = pName === "linkedin" ? "LinkedIn" : pName === "x" ? "X (Twitter)" : pName;
      await c.env.DB.prepare(
...
scheduleRouter.post("/", async (c) => {
  ...
  const validPlatforms = [];
  for (const p of platforms) {
    const dbName = p === "linkedin" ? "LinkedIn" : p === "x" ? "X (Twitter)" : p;
    const isX = p === "x";
    ...
    if (!rec) {
      if (isX && c.env.X_API_KEY) {
        ...
        await c.env.DB.prepare("INSERT INTO Platforms (id, user_id, name, credentials_ref) VALUES (?, ?, ?, ?)").bind(pid, user.id, "X (Twitter)", ref).run();
  ...
  for (const platform of validPlatforms) {
    ...
    await c.env.DB.prepare("INSERT INTO PostPlatforms (id, post_id, platform_id, status) VALUES (?, ?, ?, ?)").bind(postPlatformId, postId, platform.id, "scheduled").run();
...
var queueConsumer = /* @__PURE__ */ __name(async (batch, env) => {
  ...
  for (const message of batch.messages) {
    ...
    try {
      ...
      if (!credsString)
        throw new Error(`Credentials missing for platform ${platform.name}`);
      ...
      await env.DB.prepare("UPDATE PostPlatforms SET status = ?, response = ? WHERE id = ?").bind("success", JSON.stringify(response), job.postPlatformId).run();
      ...
      if (remaining.length === 0) {
        await env.DB.prepare("UPDATE Posts SET status = ?, updated_at = ? WHERE id = ?").bind("published", (/* @__PURE__ */ new Date()).toISOString(), job.postId).run();
      ...
      if (job?.postPlatformId) {
        await env.DB.prepare("UPDATE PostPlatforms SET status = ?, response = ? WHERE id = ?").bind("failed", JSON.stringify({ error: e.message }), job.postPlatformId).run();
      }
    }
  }
}, "queueConsumer");
...
// .wrangler/tmp/bundle-gvX0wq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gvX0wq/middleware-loader.entry.ts
