...
// .wrangler/tmp/bundle-eVnOFz/checked-fetch.js
...
// .wrangler/tmp/bundle-eVnOFz/strip-cf-connecting-ip-header.js
...
postsRouter.post("/", async (c) => {
  ...
  const { content, media_id, scheduled_at, platforms } = await c.req.json();
  ...
  for (const p of platforms) {
    ...
    if (!platformRecord) {
      return c.json({ error: `Platform ${dbName} not connected. Try reconnecting.` }, 400);
    }
    ...
    validPlatforms.push(platformRecord.id);
  }
  ...
  try {
    ...
    ).bind(postId, user.id, content, media_id || null, status, scheduledTime, now, now).run();
    const batch = [];
    const jobs = [];
    for (const platformId of validPlatforms) {
      ...
      batch.push(
        ...
        ).bind(postPlatformId, postId, platformId, status === "publishing" ? "pending" : "scheduled")
      );
      jobs.push({
        postPlatformId,
        postId,
    ...
    if (status === "publishing") {
      for (const job of jobs) {
        await c.env.PUBLISH_QUEUE.send(job);
      }
    }
    return c.json({ success: true, postId, status });
  } catch (e) {
...
var AIService = class {
  ...
  async generateCaption(topic, platform) {
    const response = await this.ai.run("@cf/meta/llama-3-8b-instruct", {
      messages: [
        { role: "system", content: "You are an expert copywriter for a premium business. Write an engaging social media post based on the user prompt. Do not mention software, updates, or digital strategy. Keep it natural, include 2 emojis, and add 3-4 short, highly relevant hashtags at the end." },
...
${content}`;
    const response = await this.ai.run("@cf/meta/llama-3-8b-instruct", {
      messages: [
        { role: "system", content: "You are an expert social media formatter." },
...
aiRouter.post("/generate", async (c) => {
  const { prompt, platform } = await c.req.json();
  if (!prompt) {
    return c.json({ error: "Prompt is required" }, 400);
  }
  try {
    const aiService = new AIService(c.env.AI);
    const result = await aiService.generateCaption(prompt, platform || "multiple platforms");
...
var LinkedInAdapter = class {
  async publish(params) {
    ...
    if (!profileResponse.ok) {
      ...
      console.log("LinkedIn API Error (Profile):", errorText);
      throw new Error(`Invalid token or profile fetch failed: ${errorText}`);
    }
...
// .wrangler/tmp/bundle-eVnOFz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-eVnOFz/middleware-loader.entry.ts
