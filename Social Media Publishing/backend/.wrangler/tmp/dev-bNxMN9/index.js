...
// .wrangler/tmp/bundle-D83MZ1/strip-cf-connecting-ip-header.js
...
postsRouter.post("/", async (c) => {
  ...
  for (const p of platforms) {
    ...
    if (!platformRecord) {
      if (dbName === "X (Twitter)" && c.env.X_API_KEY && c.env.X_ACCESS_TOKEN) {
    ...
    } else if (dbName === "X (Twitter)") {
      const existingCreds = await c.env.KV.get(platformRecord.credentials_ref);
      if (existingCreds) {
        const parsed = JSON.parse(existingCreds);
        if (parsed.accessToken === "mock_token_for_seeding" && c.env.X_API_KEY) {
          console.log("Refreshing stale X mock credentials with real env credentials...");
          await c.env.KV.put(platformRecord.credentials_ref, JSON.stringify({
            apiKey: c.env.X_API_KEY,
            apiSecret: c.env.X_API_SECRET,
            accessToken: c.env.X_ACCESS_TOKEN,
            accessSecret: c.env.X_ACCESS_SECRET
...
// .wrangler/tmp/bundle-D83MZ1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-D83MZ1/middleware-loader.entry.ts
