...
// .wrangler/tmp/bundle-19maMv/strip-cf-connecting-ip-header.js
...
${rawText}`,
  ...
  async editContent(content, platform2, action) {
    ...
    Your task is to take the user's provided social media post for the platform "${platform2}" and apply the following transformation:
    ...
    - Keep formatting appropriate for ${platform2}.
...
aiRouter.post("/generate", async (c) => {
  const { prompt: prompt2, platforms, audience, tone, goal, keywords, length } = await c.req.json();
  if (!prompt2) {
  ...
  try {
    ...
    const result = await aiService.generateCaption(prompt2, "general social media");
...
aiRouter.post("/edit", async (c) => {
  const { content, platform: platform2, action } = await c.req.json();
  if (!content || !platform2 || !action) {
    return c.json({ error: "Content, platform, and action are required" }, 400);
  }
  try {
    const aiService = new AIService(c.env);
    const result = await aiService.editContent(content, platform2, action);
    return c.json({ result });
  } catch (error3) {
    console.error("Cloudflare AI Error [edit]:", error3.message || error3);
    return c.json({ error: error3.message || "Failed to edit content" }, 500);
  }
});
...
// .wrangler/tmp/bundle-19maMv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-19maMv/middleware-loader.entry.ts
