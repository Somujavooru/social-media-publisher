...
// .wrangler/tmp/bundle-4EAxLV/strip-cf-connecting-ip-header.js
...
oauthRouter.get("/:platform/connect", async (c) => {
  ...
  if (platform2 === "facebook" || platform2 === "instagram") {
    ...
    console.log("FB APP ID:", appId);
    const clientId = platform2 === "facebook" ? appId : c.env.INSTAGRAM_CLIENT_ID;
    const redirectUri = encodeURIComponent(platform2 === "facebook" && process.env.FACEBOOK_REDIRECT_URI ? process.env.FACEBOOK_REDIRECT_URI : `http://localhost:8787/api/oauth/${platform2}/callback`);
    const scope = platform2 === "facebook" ? encodeURIComponent("pages_manage_posts,pages_read_engagement,public_profile") : encodeURIComponent("instagram_basic,instagram_content_publish,pages_read_engagement");
    const state = crypto.randomUUID();
...
oauthRouter.get("/:platform/callback", async (c) => {
  ...
  if (platform2 === "linkedin" || platform2 === "facebook" || platform2 === "instagram") {
    ...
    try {
      ...
      if (platform2 === "facebook" || platform2 === "instagram") {
        const clientId = platform2 === "facebook" ? process.env.FACEBOOK_APP_ID : c.env.INSTAGRAM_CLIENT_ID;
        const clientSecret = platform2 === "facebook" ? process.env.FACEBOOK_APP_SECRET : c.env.INSTAGRAM_CLIENT_SECRET;
        const redirectUri = platform2 === "facebook" && process.env.FACEBOOK_REDIRECT_URI ? process.env.FACEBOOK_REDIRECT_URI : `http://localhost:8787/api/oauth/${platform2}/callback`;
        ...
        const tokenData = await tokenResponse.json();
        if (!tokenResponse.ok) {
          console.error(`${platform2} token error:`, tokenData);
          return c.redirect(`${frontendUrl}/platforms?error=token_exchange_failed`);
        }
        const userId = "mock-user";
...
// .wrangler/tmp/bundle-4EAxLV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4EAxLV/middleware-loader.entry.ts
