...
export default function CreatePostPage() {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8787/api/ai/generate', {
        method: 'POST',
  ...
  const handleInlineEdit = async (action: string) => {
    ...
    setIsEditing(action);
    try {
      const response = await fetch('http://localhost:8787/api/ai/edit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleHashtagGenerate = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8787/api/ai/hashtags', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handlePublish = async () => {
    ...
    setPublishing(true);
    const endpoint = isScheduling ? 'http://localhost:8787/api/schedule' : 'http://localhost:8787/api/publish';
    const payload: any = {
      content: basePrompt,
  ...
  const handleDirectXPublish = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8787/api/publish/x', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer mock_token' },
